var mpwebgl, faceIndex = 1,
    ...
    expressionIndex = 0,
    lenIndex = 0,
    loadingbar, inputbar, inputbar2, footer;
...
function initEvt(){
    ...
    $("#LenChange").bind("click", onLenChange);
    $("#LenClear").bind("click", onLenClear);
}
...
function onAvatarChange() {
    ...
    if(faceId == 0){
        ...
        console.error("Ошибка загрузки лица.");
}
...
function onLenChange() {
    if (lenDirs == undefined || lenDirs == null || lenDirs.length <= 0)
        return;
    var lenId = mpwebgl.instance.loadcontactlen('items/lens/' + lenDirs[lenIndex]);
    console.log(lenDirs[lenIndex])
    if (lenId > 0) {
        if (++lenIndex == lenDirs.length)
        lenIndex = 0;
    }
    else
        console.error("load len fail");
}
function onLenClear() {
    mpwebgl.instance.unloadcontactlen();
}
