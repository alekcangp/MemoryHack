...
var Module = {
    ...
    monitorRunDependencies: function(left) {
        ...
        Module.setStatus(left ? 'Preparing... (' + (this.totalDependencies-left) + '/' + this.totalDependencies + ')' : 'Загрузка...');
    }
};
(function(factory) {
    ...
    var mpFace_, mpRender_, mpCtlAnim_, mpCtlHair_, mpCtlBeard_, mpCtlGlasses_, mpVoice_, mpCosme_, mpAnim_, mpCtlContactLen_;
    var mpFaceId_, mpHairId_, mpBeardId_, mpGlassesId_, mpExpressionId_, mpCosmeLipId_, mpCosmeCheekId_, mpCosmeEyeId_, mpVoiceId_, mpContactLenId_;
    ...
    mpwgl.prototype = {
        ...
        init_mp_param: function() {
            ...
            mpCtlContactLen_ = mpFace_.GetCtlItem(Module.MpFaceItemType.ITEM_TYPE_CONTACT_LENS);
        ...
        rtSpeakStart:function(info){
            window["mpFace_"] = mpFace_;
            var mpCtrlSpeech = mpFace_.GetCtlSpeech();
            mpCtrlSpeech.RtSpeakStart(info);
        },
        rtSpeakStop:function(){
            var mpCtrlSpeech = mpFace_.GetCtlSpeech();
            mpCtrlSpeech.RtSpeakStop();
        },
        rtSpeakGain:function(value){
            var mpCtrlSpeech = mpFace_.GetCtlSpeech();
            mpCtrlSpeech.RtSpeakGain(value);
        },
        rtSpeakQueue:function(buffer, nbyte){
            var mpCtrlSpeech = mpFace_.GetCtlSpeech();
            mpCtrlSpeech.RtSpeakQueue(buffer, nbyte);
        },
        loadcontactlen: function(name) {
            if (!_validateMpFace())
                return 0;
            if (name == undefined || name == "") {
                mess_ = "contact len not found";
                return mpContactLenId_ = 0;;
            }
            this.unloadcontactlen();
            try {
                mpContactLenId_ = mpCtlContactLen_.Create(name);
            } catch (e) {
                mess_ = e;
                mpContactLenId_ = 0;
            }
            if (mpContactLenId_ != 0) {
                mpCtlContactLen_.SetItem(mpContactLenId_, 0);
            }
            return mpContactLenId_;
        },
        unloadcontactlen: function() {
            if (mpContactLenId_ != undefined && mpContactLenId_ > 0) {
                mpCtlContactLen_.UnsetItem(mpContactLenId_);
                mpCtlContactLen_.Destroy(mpContactLenId_);
                mpContactLenId_ = 0;
            }
        },
